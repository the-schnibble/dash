...
static inline int LogPrint(const char* category, const char* format, const T1& v1, const Args&... args)
{
    ...
    CTestLogger::GetInstance().SendMsg(format, v1, args...);
    return LogPrintStr(tfm::format(format, v1, args...));
}

/**   Log error and return false */
template<typename T1, typename... Args>
static inline bool error(const char* format, const T1& v1, const Args&... args)
{
    LogPrintStr("ERROR: " + tfm::format(format, v1, args...) + "\n");
    return false;
}
...
{
    if(!LogAcceptCategory(category)) return 0;
    CTestLogger::GetInstance().SendMsg(format);
