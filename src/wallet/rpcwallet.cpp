...
{
    CAmount nAmount;
    boost::optional<bool> fLocked;
    boost::optional<int> nBCConf;
    ...
    void UpdateLocked(bool state) { fLocked = fLocked? *fLocked && state : state; }
    void UpdateConfs(int n) { nBCConf = nBCConf? min(*nBCConf, n) : n; }
    bool GetLocked() { return fLocked? *fLocked : false; }
    int GetConfs() { return nBCConf? *nBCConf : 0; }
};
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, wtx.vout)
        {
            ...
            item.UpdateLocked(fLocked);
            item.UpdateConfs(nBCDepth);
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        if (it != mapTally.end())
        {
            ...
            fLocked = (*it).second.GetLocked();
            nBCConf = (*it).second.GetConfs();
        ...
        else
        {
            ...
            obj.push_back(Pair("instantlock",   fLocked));
            obj.push_back(Pair("confirmations", nBCConf));
            if (!fByAccounts)
    ...
    if (fByAccounts)
    {
        for (map<string, tallyitem>::iterator it = mapAccountTally.begin(); it != mapAccountTally.end(); ++it)
        {
            ...
            bool fLocked = (*it).second.GetLocked();
            int nBCConf = (*it).second.GetConfs();
            UniValue obj(UniValue::VOBJ);
